...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    *data << uint8(bg->GetPlayersCountByTeam(HORDE));
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        ObjectGuid playerGUID = itr2->first;
...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, uint64 guid, Player* player, BattlegroundTypeId bgTypeId)
{
	...
	PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(0, player->getLevel());
	if (!bracketEntry)
		return;
...
BGFreeSlotQueueType& BattlegroundMgr::GetBGFreeSlotQueueStore(BattlegroundTypeId bgTypeId)
{
...
{
	BGFreeSlotQueueType& queues = bgDataStore[bgTypeId].BGFreeSlotQueue;
	for (BGFreeSlotQueueType::iterator itr = queues.begin(); itr != queues.end(); ++itr)
