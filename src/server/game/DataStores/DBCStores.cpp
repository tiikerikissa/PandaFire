...
void LoadDBCStores(const std::string& dataPath)
{
	...
	LoadDBC(availableDbcLocales, bad_dbc_files, sChrSpecializationsStore, dbcPath, "ChrSpecialization.dbc");

	// prepare fast data access to bit pos of talent ranks for use at inspecting
	{
		// now have all max ranks (and then bit amount used for store talent ranks in inspect)
		for (uint32 j = 0; j < sChrSpecializationsStore.GetNumRows(); j++)
		{
			ChrSpecializationsEntry const* specializationInfo = sChrSpecializationsStore.LookupEntry(j);
			...
			sSpecializationClassStore[specializationInfo->classId][specializationInfo->tabId] = j;
		}
	}
	...
	if (!sAreaStore.LookupEntry(5491) ||     // Last area (areaflag) added in 5.4 (17399)
		...
		!sGemPropertiesStore.LookupEntry(2467) ||     // Last gem property added in 5.4.0 (17399)
		!sMapStore.LookupEntry(1173) ||     // Last map added in 5.4.0 (17399)
		!sSpellStore.LookupEntry(163227))      // Last spell added in 5.4.0 (17399)
	{
