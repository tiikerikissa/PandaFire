...
class ObjectMgr
{
    ...
    public:
		...
		typedef UNORDERED_MAP<uint32, CurrencyOnGobEntry> CurOnGobContainer;
		...
		CurrencyOnGobEntry const* GetCurrencyOnGobEntry(uint32 id) const
		{
			CurOnGobContainer::const_iterator itr = _curOnGobStore.find(id);
			if (itr != _curOnGobStore.end())
				return &itr->second;
			return NULL;
		}
    ...
    private:
		...
		CurOnGobContainer _curOnGobStore;
