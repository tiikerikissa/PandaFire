...
void ObjectMgr::LoadCurrencyOnGob()
{
	uint32 oldMSTime = getMSTime();

	_curOnGobStore.clear();

	uint32 count = 0;

	QueryResult result = WorldDatabase.Query("SELECT `Gameobject_id`, `CurrencyId1`,  `CurrencyId2`,  `CurrencyId3`, `CurrencyCount1`, `CurrencyCount2`, `CurrencyCount3` FROM `Gameobject_loot_currency`");

	if (!result)
	{
		sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 gameobject loot currency definitions. DB table `Gameobject_loot_currency` is empty.");
		return;
	}
	...
	do
	{
		...
		uint32 gameobject_id = fields[0].GetUInt32();

		CurrencyOnGobEntry currOnGob;
		currOnGob.currencyId1 = fields[1].GetUInt16();
		currOnGob.currencyId2 = fields[2].GetUInt16();
		currOnGob.currencyId3 = fields[3].GetUInt16();
		currOnGob.currencyCount1 = fields[4].GetInt32();
		currOnGob.currencyCount2 = fields[5].GetInt32();
		currOnGob.currencyCount3 = fields[6].GetInt32();

		if (!GetGameObjectTemplate(gameobject_id))
		{
			sLog->outError(LOG_FILTER_SQL, "Table `Gameobject_loot_currency` have data for not existed gameobject entry (%u), skipped", gameobject_id);
			continue;
		}

		if (currOnGob.currencyId1)
		{
			if (!sCurrencyTypesStore.LookupEntry(currOnGob.currencyId1))
			{
				sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", currOnGob.currencyId1);
				continue;
			}
		}

		if (currOnGob.currencyId2)
		{
			if (!sCurrencyTypesStore.LookupEntry(currOnGob.currencyId2))
			{
				sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", currOnGob.currencyId2);
				continue;
			}
		}

		if (currOnGob.currencyId3)
		{
			if (!sCurrencyTypesStore.LookupEntry(currOnGob.currencyId3))
			{
				sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", currOnGob.currencyId3);
				continue;
			}
		}

		_curOnGobStore[gameobject_id] = currOnGob;

		++count;
	} while (result->NextRow());

	sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u gameobject loot currency in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
