...
uint32 LootStore::LoadLootTable()
{
    ...
    //                                                0      1     2         3                    4         5        6              7
    QueryResult result = WorldDatabase.PQuery("SELECT entry, item, currency, ChanceOrQuestChance, lootmode, groupid, mincountOrRef, maxcount FROM %s", GetName());
    ...
    do
    {
        ...
        uint16 lootmode            = fields[4].GetUInt16();
        uint8  group               = fields[5].GetUInt8();
        int32  mincountOrRef       = fields[6].GetInt32();
		...
		LootStoreItem storeitem = LootStoreItem(item, currency, chanceOrQuestChance, lootmode, group, mincountOrRef, maxcount);
    ...
    for (std::set<uint64>::iterator i = PlayersLooting.begin(); i != PlayersLooting.end(); i = i_next)
    {
        ...
        if (Player* player = ObjectAccessor::FindPlayer(*i))
        {
            ...
            if (pq != PlayerQuestItems.end() && pq->second)
            {
                ...
                if (j < pql.size())
                    player->SendNotifyLootItemRemoved(items.size()+j, Guid);
            }
        }
        else
            PlayersLooting.erase(i);
