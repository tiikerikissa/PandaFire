...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
	...
	if (!forced)
	{
		...
		{
			sLog->outError(LOG_FILTER_OPCODES, "Prevented sending disabled opcode %s to %s", GetOpcodeNameForLogging(packet->GetOpcode(), true).c_str(), GetPlayerName(false).c_str());
			return;
		}
	}
...
void WorldSession::LogUnexpectedOpcode(WorldPacket* packet, const char* status, const char *reason)
{
	sLog->outError(LOG_FILTER_OPCODES, "Received unexpected opcode %s Status: %s Reason: %s from %s",
		GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str(), status, reason, GetPlayerName(false).c_str());
}
...
void WorldSession::LogUnprocessedTail(WorldPacket* packet)
{
	...
	sLog->outError(LOG_FILTER_OPCODES, "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
		uint32(packet->rpos()), uint32(packet->wpos()), GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str(), GetPlayerName(false).c_str());
		...
		_recvQueue.next(packet, updater))
	{
		...
		try
		{
			...
			case STATUS_AUTHED:
				...
				break;
			case STATUS_NEVER:
				sLog->outError(LOG_FILTER_OPCODES, "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str()
					, GetPlayerName(false).c_str());
				break;
			case STATUS_UNHANDLED:
				sLog->outError(LOG_FILTER_OPCODES, "Received not handled opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), false).c_str()
					, GetPlayerName(false).c_str());
				break;
			}
		}
...
void WorldSession::Handle_NULL(WorldPacket& recvPacket)
{
	sLog->outError(LOG_FILTER_OPCODES, "Received unhandled opcode %s from %s"
		, GetOpcodeNameForLogging(recvPacket.GetOpcode(), false).c_str(), GetPlayerName(false).c_str());
}
