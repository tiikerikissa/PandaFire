...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    switch(pkt->GetOpcode())
	{
		...
		default:
		{
			sLog->outError(LOG_FILTER_GENERAL, "S->C: %s", GetOpcodeNameForLogging(pkt->GetOpcode(), true).c_str());
		}
	}
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
	...
	if (m_Session)
		sLog->outError(LOG_FILTER_GENERAL, "C->S: %s", opcodeName.c_str(), true);
    ...
    try
    {
        switch (opcode)
        {
            ...
            default:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_OPCODES, "No defined handler for opcode %s sent by %s", GetOpcodeNameForLogging(new_pct->GetOpcode(), true).c_str(), m_Session->GetPlayerName(false).c_str());
                    return 0;
                }
