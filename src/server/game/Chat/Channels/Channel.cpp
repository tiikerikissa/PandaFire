...
void Channel::Say(uint64 p, const char *what, uint32 lang)
{
	...
	WorldPacket data;
	ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), p, p, what, chatTag, "", "", 0, isGM, m_name);
...
{
    data->Initialize(SMSG_CHANNEL_NOTIFY, 1 + m_name.size());
