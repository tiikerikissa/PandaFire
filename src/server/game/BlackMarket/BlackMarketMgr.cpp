...
void BlackMarketMgr::BuildBlackMarketAuctionsPacket(WorldPacket& data, uint32 guidLow)
{
	...
	for (BMAuctionEntryMap::const_iterator itr = GetAuctionsBegin(); itr != GetAuctionsEnd(); ++itr)
	{
		...
		data << uint32(0); //unk 12
		data << uint32(auction->bm_template->itemCount); //stack count 16
		data << uint32(auction->id); // auction id 0
		data << uint32(0); //unk 56
		data << uint32(auction->bm_template->itemEntry); //item id 8
		data << uint64(0); //unk 24 28
		data << uint64(auction->bid); // price 40 44
		data << uint64(0); //unk 32 36
		data << uint32(auction->bm_template->seller); //seller 4
		data << uint32(auction->TimeLeft()); //time left 48
