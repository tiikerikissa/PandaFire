...
{
    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: Received SMSG_MINIMAP_PONG");
    ...
    //sLog->outDebug(LOG_FILTER_GENERAL, "Received opcode SMSG_MINIMAP_PONG X: %f, Y: %f", x, y);
...
void WorldSession::HandleRequestPartyMemberStatsOpcode(WorldPacket& recvData)
{
	...
	uint8 bitOrder[8] = { 7, 4, 0, 1, 3, 6, 2, 5 };
    recvData.ReadBitInOrder(Guid, bitOrder);

    recvData.FlushBits();

	uint8 byteOrder[8] = { 3, 6, 5, 2, 1, 4, 0, 7 };
    recvData.ReadBytesSeq(Guid, byteOrder);
