...
void WorldSession::HandleLfgJoinOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_LFG_JOIN [" UI64FMTD "] roles: %u, Dungeons: %u, Comment: %s", GetPlayer()->GetGUID(), roles, uint8(newDungeons.size()), 0);
    sLFGMgr->Join(GetPlayer(), uint8(roles), newDungeons, 0);
}
