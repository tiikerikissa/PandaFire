...
void WorldSession::SendAuctionHello(uint64 guid, Creature* unit)
{
	...
	WorldPacket data(SMSG_AUCTION_HELLO_RESPONSE, 1 + 1 + 8 + 4);
...
{
	WorldPacket data(SMSG_AUCTION_OWNER_BID_NOTIFICATION, 4 + 4 + 4 + 4 + 4 + 8 + 1);
	data << uint32(0);
...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
	...
	{
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleAuctionSellItem - Player %s (GUID %u) attempted to sell item with higher price than max gold amount.", _player->GetGUIDLow());
	...
	if (itemCount == 1 && item->GetCount() == count[0])
	{
		...
		sLog->outInfo(LOG_FILTER_NETWORKIO, "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s entry %u (guid %d) "
			"to auctioneer %u with count %u with initial bid %u with buyout %u and with time %u (in sec) in auctionhouse %u", 
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleAuctionListBidderItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneer)));
        recvData.rfinish();
        return;
    }
...
void WorldSession::HandleAuctionListOwnerItems(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleAuctionListOwnerItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneer)));
        return;
    }
...
void WorldSession::HandleAuctionListItems(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleAuctionListItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneer)));
        return;
    }
