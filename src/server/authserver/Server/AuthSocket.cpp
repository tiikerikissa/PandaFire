        ...
        if (res2)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    sLog->outDebug(LOG_FILTER_AUTHSERVER, "database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (_expversion == NO_VALID_EXP_FLAG)
    {
        ...
        sLog->outDebug(LOG_FILTER_AUTHSERVER, "Client with invalid version, patching is not implemented");
        socket().shutdown();
        return true;
    }
